...
app.get('/stories', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: '스토리 데이터를 불러오는 데 실패했습니다.' });
    }
});
...
app.get('/posts', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM one_s_story');
        res.json(result.rows);
    } catch (err) {
        console.error('Error fetching posts:', err);
        res.status(500).json({ error: '게시물 데이터를 불러오는 데 실패했습니다.' });
    }
});
...
app.get('/board/:story_id/post/:geul_id', async (req, res) => {
    ...
    if (!story_id || isNaN(parseInt(story_id)) || !geul_id || isNaN(parseInt(geul_id))) {
        console.error('Invalid story_id or geul_id:', story_id, geul_id);
        return res.status(400).json({ error: '유효하지 않은 story_id 또는 geul_id입니다.' });
    }
